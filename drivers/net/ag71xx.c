...
static int ag71xx_ether_mii_read(struct mii_bus *dev, int addr, int reg)
{
	printk("ag71xx_ether_mii_read\n");
	return 0;
}

static int ag71xx_ether_mii_write(struct mii_bus *dev, int addr, int reg, u16 val)
{
	printk("ag71xx_ether_mii_write\n");
	return 0;
}
...
static int ag71xx_ether_init(struct eth_device *edev)
{
	...
	ag71xx_wr(etdev, AG71XX_REG_RX_DESC, virt_to_phys(etdev->fifo_rx[0]));
...
static int ag71xx_probe(struct device_d *dev)
{
	...
	__raw_writel(base + AR933X_ETHSW_CLOCK_CONTROL_REG, rd);
        ...
        if(__raw_readl(base + AR933X_RESET_REG_RESET_MODULE) != 0)
            __raw_writel(base + AR933X_RESET_REG_RESET_MODULE, 0);
#endif
