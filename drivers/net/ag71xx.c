...
static int ag71xx_probe(struct device_d *dev)
{
	...
	for (i = 0; i < NO_OF_TX_FIFOS; i++) {
		ether_dev->fifo_tx[i]->next_desc = (i == NO_OF_TX_FIFOS - 1) ? ether_dev->fifo_tx[0] : ether_dev->fifo_tx[i + 1];
	}

	ether_dev->fifo_rx = dma_alloc_coherent(NO_OF_TX_FIFOS * sizeof(ag7240_desc_t), DMA_ADDRESS_BROKEN);
	for (i = 0; i < NO_OF_RX_FIFOS; i++) {
		ether_dev->fifo_rx[i]->next_desc = (i == NO_OF_RX_FIFOS - 1) ? ether_dev->fifo_rx[0] : ether_dev->fifo_rx[i + 1];
	}
